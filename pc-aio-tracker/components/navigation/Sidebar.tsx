...
interface SidebarProps {
  ...
  activeEngine: "media" | "commerce" | "creative" | "signal" | "sku" | null;
  onSelectHome: () => void;
  onSelectEngine: (
  ...
  return (
    <aside
      ...
      <div className="pt-4 pb-4 px-2 space-y-1 border-b border-slate-800/80">
        ...
        <button
          onClick={() => onSelectEngine("sku")}
          title="PID & SKU Performance"
          className={`w-full rounded-xl text-sm flex items-center font-semibold transition-all ${isSidebarOpen ? "px-3.5 py-3 gap-3" : "justify-center p-3"} ${activeEngine === "sku" ? "bg-indigo-600 text-white shadow-md" : "hover:bg-[#111827] text-slate-400"}`}>
          <svg
            className="w-5 h-5 shrink-0"
            fill="none"
            stroke="currentColor"
          ...
          {isSidebarOpen && (
            <span className="truncate">PID & SKU Analytics</span>
          )}
        </button>
      </div>
